...
void sine_transform(matrix<double> &in, const double multiplier,
                    const double rel_phase, bool preserve_zero) {
  if (preserve_zero) {
...
vec2 calc_pixel_widths(size_t x, size_t y, const vec2 center,
                       const double zoom) {
  double dx = 2 / zoom;
  double dy = 2 / zoom;
  if (x > y) {
...
vec4 calc_bounds(size_t x, size_t y, const vec2 center, const double zoom) {
  double dx = 2 / zoom;
  double dy = 2 / zoom;
  if (x > y) {
    /* widescreen image */
...
double fractal_base::iterate_cell(const complex pos) {
  ...
  if (subsample) {
    double out[] = {0, 0, 0, 0};
    if (is_julia) {
      out[0] = fractal_cell(pos + complex(-pixel_width_x, 0), c, max_iterations,
                            smooth, polynomial, custom_polynomial);
      out[1] = fractal_cell(pos + complex(pixel_width_x, 0), c, max_iterations,
                            smooth, polynomial, custom_polynomial);
      out[2] = fractal_cell(pos + complex(0, -pixel_width_y), c, max_iterations,
                            smooth, polynomial, custom_polynomial);
      out[3] = fractal_cell(pos + complex(0, pixel_width_y), c, max_iterations,
                            smooth, polynomial, custom_polynomial);
    } else {
      out[0] =
          fractal_cell(complex(0, 0), pos + complex(-pixel_width_x, 0),
                       max_iterations, smooth, polynomial, custom_polynomial);
      out[1] =
          fractal_cell(complex(0, 0), pos + complex(pixel_width_x, 0),
                       max_iterations, smooth, polynomial, custom_polynomial);
      out[2] =
          fractal_cell(complex(0, 0), pos + complex(0, -pixel_width_y),
                       max_iterations, smooth, polynomial, custom_polynomial);
      out[3] =
          fractal_cell(complex(0, 0), pos + complex(0, pixel_width_y),
                       max_iterations, smooth, polynomial, custom_polynomial);
...
bool fractal_base::process_line(const line &l) {
  ...
  const vec_ull diff =
      ((start - end) != vec_ull{0, 0}) ? (end - start).unitV() : vec_ull{0, 0};
...
void fractal_base::set_zoom(vec2 center, double zoom) {
  ...
  auto wid = calc_pixel_widths(iterations.x(), iterations.y(), center, zoom);
  pixel_width_x = wid[0];
  pixel_width_y = wid[1];
}
...
split_rectangle fractal_base::process_rectangle(rectangle r) {
  ...
  if (!edges_equal && shortest_edge > 1) {
    // must be careful how we round up and down because rectangles are inclusive
    // on all bounds
    return {
        true,
        {
            rectangle(r.xmin, (r.xmin + r.xmax) / 2, r.ymin,
                      (r.ymin + r.ymax) / 2),
            rectangle((r.xmin + r.xmax) / 2, r.xmax, r.ymin,
                      (r.ymin + r.ymax) / 2),
            rectangle(r.xmin, (r.xmin + r.xmax) / 2, (r.ymin + r.ymax) / 2,
                      r.ymax),
            rectangle((r.xmin + r.xmax) / 2, r.xmax, (r.ymin + r.ymax) / 2,
                      r.ymax),
        },
    };
...
fractal_base::fractal_base(const size_t w, const size_t h)
    : iterations(w, h, NOT_DEFINED), grid_mask(w, h, 0), pixel_width_x(2 / w),
      pixel_width_y(2 / h) {}
...
vec4 fractal_base::get_bounds() const { return bounds; }

void fractal_base::set_bounds(vec4 bounds) { this->bounds = bounds; }

void fractal_base::read_config(const fractal_info &cfg) {
...
fractal_base::fractal_base(const fractal_base &rhs)
    : iterations(rhs.iterations), grid_mask(rhs.grid_mask),
      pixel_width_x(rhs.pixel_width_x), pixel_width_y(rhs.pixel_width_y),
      custom_polynomial(rhs.custom_polynomial), polynomial(rhs.polynomial),
      max_iterations(rhs.max_iterations), do_grid(rhs.do_grid),
      is_julia(rhs.is_julia), smooth(rhs.smooth),
      do_sine_transform(rhs.do_sine_transform), subsample(rhs.subsample),
      mul(rhs.mul), c(rhs.c) {}
};
