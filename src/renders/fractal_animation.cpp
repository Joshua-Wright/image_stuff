...
#pragma omp parallel for schedule(static) private(grid)
  for (size_t i = 0; i < n_frames + 1; i++) {
    ...
    scale_grid(grid);
    //        color_write_image(grid, [t, cmap](double x1) { return cmap(x1, t); }, out_filename,
    //        false);
  ...
  std::cout << "ffmpeg -framerate 60 -i " << output_folder << "out_frame_%05d.png " << output_folder
            << "output.mp4" << std::endl;
