...
auto func_standard = [](const complex &z, const complex &c) {
  return pow(z, 2) + c;
};
auto func_cubic = [](const complex &z, const complex &c) {
  return pow(z, 3) + c;
};
//    auto func_inv_c = [](const complex &z, const complex &c) { return pow(z,
//    2) + 1.0 / c; };
auto func_inv_c = [](const complex &z, const complex &c) {
  return pow(z, 2) + 1.0 / (c - 1.0);
};
auto func_inv_c_parabola = [](const complex &z, const complex &c) {
  return pow(z, 2) + 1.0 / c + 0.25;
};
auto func_quadratic_rational = [](const complex &z, const complex &c) {
  return pow(z, 2) + pow(c, 2) / (pow(c, 4) - 0.25);
};
...
auto func_inv_lambda = [](const complex &z, const complex &c) {
  return 1.0 / (z * (c - 1.0));
};
...
double fractal_cell_(complex z, const complex &c, const size_t max_iterations,
                     const bool smooth, const Func func = func_standard) {
...
void sine_transform(matrix<double> &in, const double multiplier = 1,
                    const double rel_phase = 0, bool preserve_zero = true);
...
struct rectangle {
  ...
  rectangle(const size_t x_min, const size_t x_max, const size_t y_min,
            const size_t y_max)
      : xmin(x_min), xmax(x_max), ymin(y_min), ymax(y_max) {}

  std::array<line, 4> get_sides() const {
    return std::array<line, 4>{
        line{{xmin, ymin}, {xmax, ymin}}, line{{xmin, ymin}, {xmin, ymax}},
