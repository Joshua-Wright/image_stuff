...
colormap_3d_cosine::colormap_3d_cosine(const vec3 &a, const vec3 &b,
                                       const vec3 &c, const vec3 &d)
    : a(a), b(b), c(c), d(d) {}

/* this is just to make writing out get_rgb() easier */
vec3 cos(vec3 in) { return {cos(in[0]), cos(in[1]), cos(in[2])}; }

RGB colormap_3d_cosine::operator()(const double x) const {
  vec3 color = 256.0 * (a + b * cos(c * x * 64 + d));
  return {(unsigned char)color[0], (unsigned char)color[1],
...
const std::map<std::string, std::function<RGB(const double)>> default_colormaps{
    ...
    {"3d-cosine", colormap_3d_cosine({0.5, 0.5, 0.5}, {0.5, 0.5, 0.5},
                                     {0.15, 0.15, 0.15}, {3.0, 3.6, 4.0})},
};
...
colormap read_colormap_from_string(const std::string &spec,
  ...
  if (spec[0] == '{') {
    // parse json colormap
    json j = json::parse(spec);
    if (j["type"] == "cubic") {
      json data = j["data"];
      // format: {"type":"cubic","data":[ [x1,[r,g,b]],[x2,[r,g,b]] ] }
      std::vector<std::pair<double, vec3>> xps;
      for (json::iterator it_x = data.begin(); it_x != data.end(); it_x++) {
        double x = (*it_x)[0];
        vec3 color{(*it_x)[1][0], (*it_x)[1][1], (*it_x)[1][2]};
        xps.emplace_back(x, color);
      }
      return colormap(colormap_interpolant(xps), steps);
    }
...
colormap_interpolant::colormap_interpolant(
    std::vector<std::pair<double, vec3>> xps) {
  std::sort(xps.begin(), xps.end(),
...
colormap::colormap(const std::vector<RGB> &color_data)
